...
tausche(int64_t a, int64_t b, int64_t pos)
{
...
fseek(verwalt,a*sizeof(dateiinfo),SEEK_SET);
fseek(speich, a*bz*sizeof(char),SEEK_SET);
int64_t s = 0;
int64_t t = 0;
while(s < b)
{
...
fwrite(&dateiinfo, sizeof(dateiinfo), 1, verwalt);
s = s+1;
fseek(verwalt, (a+s)*sizeof(dateiinfo),SEEK_SET);

fread(p, bz*sizeof(char), 1, speich);
fseek(verwalt, (pos+t)*bz*sizeof(char),SEEK_SET);
fwrite(p, bz*sizeof(char), 1, speich);
t = t+1;
fseek(verwalt, (a+t)*sizeof(dateiinfo),SEEK_SET);
}
...
fseek(verwalt,b*sizeof(dateiinfo),SEEK_SET);
fseek(speich, b*bz*sizeof(char),SEEK_SET);
int64_t l = 0;
while(l < a)
{
