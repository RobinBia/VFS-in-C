...
speich = fopen(pfadcpy2,"w+");
	if(speich == NULL)
	{
	printf("Das Anlegen des VFS war nicht moglich - Aus Grunden,die das Hostsystem und nicht Ihr Programm verursacht hat!\n");
	return 1;	
	}
}
...
f = fwrite(puffer, bz*sizeof(char), 1, speich);
if(f == 0){return 66;}
u = u+1;
}
free(puffer);
...
while(s < bc)
{
f = fwrite(dateiinfo, sizeof(struct d) + sizeof(char), 1, verwalt);
if(f == 0){return 66;}
s = s+1;
}
...
f = fread(dateiinfo,sizeof(struct d),1,verwalt);
if(f == 0){return 66;}
...
f = fread(dateiinfo,sizeof(struct d) + max*sizeof(char),1,verwalt);
if(f == 0){return 66;}
...
//Hier wird eine komplett neue .structure geschrieben werden, sodass ein größerer Dateiname, sofern dieser erforderlich ist, hineinpasst.
...
f = fwrite(dateiinfo, sizeof(struct d)+ nl*sizeof(char), 1, verwalt);//Schreiben des nun größeren Objekts in den Stream
if(f == 0){return 66;}
free(dateiinfo);
i = i+1;
}
...
f = fread(dateiinfo,sizeof(struct d) + newmax*sizeof(char), 1, verwalt);
if(f == 0){return 66;}
...
int64_t blockpos = pos; //Zähler, um Dateiinfo zwischendurch mit der aktuellen Blocknummer zu initialisieren
	...
	fread(p, bz*sizeof(char), 1, temp); //Hier keine Überprüfung als Ausnahme, da letzter Block nicht komplett gelesen werden kann.
	f = fwrite(p, bz*sizeof(char), 1, speich );
	if(f == 0){return 66;}
	dateiinfo->blocknummer = blockpos;
	f = fwrite(dateiinfo, sizeof(struct d)+ newmax*sizeof(char), 1, verwalt);
	if(f == 0){return 66;}
	m = m+1;
	blockpos = blockpos+1;
	}
...
while(fread(dateiinfo, sizeof(struct d) + max*sizeof(char), 1, verwalt) == 1)
{
	...
	while(bzaehl < dateiinfo->dateigroesse)
		{
		//memset(p,' ', 1);
		f = fread(p, sizeof(char), 1, speich);
		if(f == 0){return 66;}
...
while(fread(dateiinfo,sizeof(struct d)+max*sizeof(char), 1, verwalt) == 1)
{
	...
	f = fwrite(dateiinfo, sizeof(struct d)+max*sizeof(char), 1, verwalt);
	if(f == 0){return 66;}
...
//_______Reduktion der Strukturdateigröße_______________________________________________________________________________________________________________________________________________________
rewind(verwalt);
...
f = fwrite(dateiinfo, sizeof(struct d) + oldmax*sizeof(char), 1, verwalt);//Schreiben des neuen Objekts in den Stream
if(f == 0){return 66;}
free(dateiinfo);
i = i+1;
}
...
f = fread(p, bz*sizeof(char), 1, speich);
if(f == 0){return 66;}
...
////argv[0] == "vfs", argv[1] == "archiv",argv[2] == "operation",argv[3,4] == "parameter",/////////// main-Funktion //////////////////////////////////////////////////////////////////////
