...
dateiinfo = malloc(sizeof(struct d)+ 10*sizeof(char));
int64_t max = 10;
dateiinfo->max = 10;
...
int64_t premax = dateiinfo.max;
printf("premax ist hier %lld",premax);
...
while(i < bc)
{
dateiinfo = dat[i]; //Objekt an der Stelle i in den Struct schreiben
dateiinfo.max = nl;
dateiinfo.dateiname = realloc(dateiinfo.dateiname, dateiinfo.max*sizeof(char));//Objekt größer machen
fwrite(&dateiinfo, sizeof(dateiinfo), 1, verwalt);
dateiinfo.max = premax; //Alte Dateiinfogröße zurückholen
dateiinfo.dateiname = realloc(dateiinfo.dateiname, dateiinfo.max*sizeof(char)); //Objekt wieder kleiner machen
i = i+1;
}
dateiinfo.max = nl; //Entgültige größe sichern
dateiinfo.dateiname = realloc(dateiinfo.dateiname, dateiinfo.max*sizeof(char));
fclose(verwalt);
...
strncpy(dateiinfo.dateiname, target, nl);
...
fread(&dateiinfo,sizeof(dateiinfo),1,verwalt);
dateiinfo.dateiname = malloc(dateiinfo.max*sizeof(char));
rewind(verwalt);
